...
export async function GET() {
  ...
  for (const key of ['issuedAt', 'activatedAt', 'expiryDate']) {
  ...
  reference.columns = [
    ...
    { header: 'Card type codes', key: 'typeCode', width: 22 },
    { header: 'Card type names', key: 'typeName', width: 30 },
  ];
  reference.getRow(1).font = { bold: true };

  const maxRows = Math.max(CARD_STATUSES.length, locations.length, cardTypes.length);
